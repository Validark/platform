...
# Usage:
#   get_config <node> <option>
#
# where <node> is either node number (1,2,3) or 'seed'
# and <option> is a dot-separated path to the option
#
# Example:
#   get_config seed core.rpc.port
function get_config() {
    # We use `jq` because it's much faster than `yarn dashmate config get`
    jq ".configs.${CONFIG}_$1.$2" <"${HOME}"/.dashmate/config.json
}
...
DPNS_CONTRACT_ID=$(get_config 1 platform.dpns.contract.id)
...
CORE_RPC_PORT=$(get_config seed core.rpc.port)
CORE_RPC_USER=$(get_config seed core.rpc.user)
CORE_RPC_PASSWORD=$(get_config seed core.rpc.password)
PLATFORM_RPC_PORT=$(get_config 1 platform.dapi.envoy.http.port)
